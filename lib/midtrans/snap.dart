...
class _SnapScreenState extends State<SnapScreen> {
  ...
  _loadHtmlFromAssets() {
    webViewController?.loadFlutterAsset(Uri.dataFromString('''<html>
      <head>
  ...
  _handleResponse(message) {
    try {
      Midtrans midtrans = Midtrans.fromString(message);
      var title, desc;
      if (Platform.isAndroid) {
        switch (message) {
          case 'ok':
...
class Midtrans {
  ...
  Midtrans.fromString(String message) {
    RegExp codeExp = RegExp(r'"status_code".*');
    RegExpMatch? codeStr = codeExp.firstMatch(message);
    String? code = RegExp(r'\d+;$').stringMatch(codeStr.toString());
    ...
    RegExp typeExp = RegExp(r'"payment_type".*');
    RegExpMatch? typeStr = typeExp.firstMatch(message);
    String? typeStrOut =  RegExp(r'"?\w+"?;').stringMatch(typeStr.toString());
    typeStrOut = typeStrOut?.replaceAll(RegExp(r'[;"]'), '');

    paymentType = MidtransPaymentTypeMap[typeStrOut];
  }
